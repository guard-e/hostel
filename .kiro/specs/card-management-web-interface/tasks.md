...
- [x] 2. Реализация DatabaseManager для работы с Firebird
  - [x] 2.1 Создать класс DatabaseManager с методами подключения
    - Реализовать методы connect(), disconnect()
    - Настроить работу с драйвером fdb для Firebird
    - _Требования: 5.4, 10.3_
  
  - [x] 2.2 Реализовать метод вызова процедуры HOSTEL_CARDEDIT
  ...
  - [x] 2.3 Реализовать метод вызова процедуры UPD_DUMPS
    - Создать метод call_upd_dumps() для обновления дампов после операций с картой
    - _Требования: 5.1_
  
  - [x] 2.4 Реализовать метод получения списка карт
  ...
  - [x] 2.5 Реализовать метод аутентификации пользователей
  ...
  - [x] 2.6 Написать property-тест для вызова процедуры HOSTEL_CARDEDIT
    - **Property 11: Корректность параметров процедуры**
    - **Validates: Requirements 5.2**

- [x] 3. Реализация модели данных Card
  - [x] 3.1 Создать класс Card с атрибутами
  ...
  - [x] 3.2 Реализовать валидацию данных карты
  ...
  - [x] 3.3 Реализовать метод parse_room для разбора номера комнаты
    - Создать статический метод parse_room() для формата (X)XYY
    - Вернуть кортеж (floor, room_number)
    - _Требования: 1.1_
  
  - [x] 3.4 Написать unit-тесты для валидации Card
    - Тестировать граничные случаи: пустые поля, некорректные даты, отрицательные числа
    - _Требования: 6.2_
  
  - [x] 3.5 Написать property-тест для валидации невалидных данных
    - **Property 2: Отклонение невалидных данных при создании**
    - **Validates: Requirements 1.2**
  
  - [x] 3.6 Написать property-тест для round-trip сериализации
    - **Property 4: Round-trip для редактирования карты**
    - **Validates: Requirements 2.1**

- [x] 4. Реализация AuthManager для управления сессиями
  - [x] 4.1 Создать класс AuthManager с методами аутентификации
  ...
  - [x] 4.2 Реализовать метод проверки прав доступа
  ...
  - [x] 4.3 Написать property-тест для аутентификации
    - **Property 19: Создание сессии при успешной аутентификации**
    - **Validates: Requirements 9.6**
  
  - [x] 4.4 Написать property-тест для отказа в доступе
    - **Property 20: Отказ в доступе при неудачной аутентификации**
    - **Validates: Requirements 9.7**

- [x] 5. Контрольная точка - Проверка базовых компонентов
...
- [x] 6. Реализация Flask routes для операций с картами
  - [x] 6.1 Создать route для главной страницы (GET /)
  ...
  - [x] 6.2 Создать route для получения списка карт (GET /cards)
    - Вызвать db_manager.get_all_cards()
    - Вернуть JSON с массивом карт
    - _Требования: 4.1, 4.2_
  
  - [x] 6.3 Создать route для создания карты (POST /cards)
  ...
  - [x] 6.4 Создать route для получения данных карты (GET /cards/<id>)
    - Вызвать db_manager.call_cardedit_procedure() с action=0
    - Вернуть JSON с данными карты
    - _Требования: 2.1_
  
  - [x] 6.5 Создать route для обновления карты (PUT /cards/<id>)
    - Валидировать входные данные
    - Вызвать db_manager.call_cardedit_procedure() с action=1
    - Вызвать db_manager.call_upd_dumps()
    - Вернуть JSON с результатом
  ...
  - [x] 6.6 Создать route для удаления карты (DELETE /cards/<id>)
    - Вызвать db_manager.call_cardedit_procedure() с action=2
    - Вызвать db_manager.call_upd_dumps()
    - Вернуть JSON с результатом
    - _Требования: 3.2, 3.3_
  
  - [x] 6.7 Написать property-тест для создания карты
    - **Property 1: Создание карты с валидными данными**
    - **Validates: Requirements 1.1, 1.3**
  
  - [x] 6.8 Написать property-тест для обновления карты
    - **Property 5: Сохранение изменений карты**
    - **Validates: Requirements 2.2, 2.3**
  
  - [x] 6.9 Написать property-тест для удаления карты
    - **Property 6: Удаление карты**
    - **Validates: Requirements 3.2, 3.3**

- [x] 7. Реализация routes для аутентификации и выбора БД
  - [x] 7.1 Создать route для выбора файла БД (GET /select-database)
    - Отобразить страницу с формой выбора файла guardee.fdb
    - _Требования: 10.1_
  
  - [x] 7.2 Создать route для обработки выбора БД (POST /select-database)
...
- [x] 10. Реализация frontend - HTML шаблоны
  ...
  - [x] 10.3 Создать шаблон выбора БД (select_database.html)
    - Создать форму выбора файла guardee.fdb
    - Добавить инструкции для пользователя
    - _Требования: 10.1_
  
  - [x] 10.4 Создать шаблон входа (login.html)
    - Создать форму с полями username и password
    - Добавить обработку сообщений об ошибках
    - _Требования: 9.2_

- [x] 11. Реализация frontend - JavaScript логика
  - [x] 11.1 Создать функцию загрузки списка карт (loadCards)
    - Выполнить AJAX GET-запрос к /cards
    - Отобразить карты в таблице
    - Добавить индикатор загрузки
    - _Требования: 4.1, 4.4, 7.4_
  
  - [x] 11.2 Создать функцию отображения формы добавления (showAddCardForm)
    - Очистить форму
    - Открыть модальное окно
    - _Требования: 1.4_
  
  - [x] 11.3 Создать функцию отображения формы редактирования (showEditCardForm)
    - Выполнить AJAX GET-запрос к /cards/<id>
    - Заполнить форму данными карты
    - Открыть модальное окно
    - _Требования: 2.1_
  
  - [x] 11.4 Создать функцию сохранения карты (saveCard)
  ...
  - [x] 11.5 Создать функцию удаления карты (deleteCard)
    - Показать диалог подтверждения
    - Выполнить AJAX DELETE-запрос при подтверждении
    - Обновить список карт при успехе
    - _Требования: 3.1, 3.2, 3.4_
  
  - [x] 11.6 Создать функцию клиентской валидации (validateCardForm)
  ...
  - [x] 11.7 Создать функцию отображения сообщений (showMessage)
    - Показать Bootstrap alert с сообщением
    - Поддержать типы: success, error, warning, info
    - _Требования: 1.3, 2.3, 3.3, 7.2_
  
  - [x] 11.8 Написать property-тест для клиентской валидации
    - **Property 13: Клиентская валидация предотвращает отправку**
    - **Validates: Requirements 6.1, 6.4**
  
  - [x] 11.9 Написать property-тест для отображения всех карт
    - **Property 8: Отображение всех карт с атрибутами**
    - **Validates: Requirements 4.1, 4.2**

- [x] 12. Реализация CSS стилей
  - [x] 12.1 Создать custom.css для дополнительных стилей
